...
def merge(img,bg,center_size,output_size):
    ...
    new_img[radius-img.shape[0]/2:radius-img.shape[0]/2+img.shape[0],radius-img.shape[1]/2:radius-img.shape[1]/2+img.shape[1],:]=img
    ...
    if np.random.rand()<0.5:
        bg = cv2.resize(bg,(output_size[0],int(output_size[1]*(np.random.rand()*2+1))),interpolation=cv2.INTER_AREA)
        y = bg.shape[0]
        bg = bg[(y-output_size[1])/2:(y+output_size[1]/2),:,:]
    else:
        bg = cv2.resize(bg,(int(output_size[0]*(np.random.rand()*2+1)),output_size[1]),interpolation=cv2.INTER_AREA)
        y = bg.shape[1]
    ...
    left = int(np.random.rand()*(w_bg-w_img-60))+30
    top = int(np.random.rand()*(h_bg-h_img-60))+30
...
def read_merge(imgpath,bgpath,des_size):
    ...
    if width > des_size:
        width = des_size
    output = merge(img, bg, (height, width), (des_size, des_size))
    return output
if __name__ == '__main__':
    # if len(sys.argv)!=5:
    #     print('Please enter like this: python merge.py pngDir bgimgDir outputDir dstSize')
    #     sys.exit(0)
    # imgDir = sys.argv[1]
    # bgDir = sys.argv[2]
    ...
    np.random.shuffle(imglst)
    np.random.shuffle(bglst)
    ...
    for e in imglst:
        ...
        outDir = os.path.join(outputDir,str(index)+'.jpg')
